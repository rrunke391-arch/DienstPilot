(() => {
  ...
  if (window.__dienstpilotDriverHomeV2) return;
  window.__dienstpilotDriverHomeV2 = true;

  const PANEL_ID = 'dpDriverHome';
  const STYLE_ID = 'dpDriverHomeV2Style';
  ...
  function addStyle() {
    ...
    style.textContent = `
      ...
      #${PANEL_ID} .dp-home-vacation-button{border:1px solid #86efac;background:#f0fdf4;color:#166534}
      #${PANEL_ID} .dp-home-vacation-button.active{border-color:#166534;background:#166534;color:#fff}
      ...
      #dpDriverVacationHost{padding:18px;border:1px solid #bbf7d0;border-radius:22px;background:#f8fffb;box-shadow:0 10px 25px rgba(22,101,52,.07)}
      #dpDriverVacationHost[hidden]{display:none!important}
      #dpDriverVacationHost .dp-driver-vacation-loading{padding:18px;text-align:center;color:#475569;font-weight:850}
  ...
  function rowsForDate(iso) {
    return dutyRows().filter((row) => row && String(row.date || '') === iso).sort((a, b) => String(a.start || '').localeCompare(String(b.start || '')));
  }
  ...
  function nextDutyDate(fromIso) {
    return [...new Set(dutyRows().filter((row) => row && !isFree(row) && /^\d{4}-\d{2}-\d{2}$/.test(String(row.date || ''))).map((row) => String(row.date)))]
      .filter((date) => date >= fromIso).sort()[0] || '';
  }
  ...
  function serviceHtml(iso) {
    ...
    const work = rows.filter((row) => !isFree(row));
    if (!work.length) {
      const text = rows.some(isFree) ? 'Für diesen Tag ist Frei eingetragen.' : (dutyRows().length ? 'Für diesen Tag ist kein Dienst eingetragen.' : 'Für diesen Fahrer ist noch kein Dienstplan geladen.');
    ...
    return `
      <div class="dp-home-head">
        ...
        <div class="dp-home-actions">
          ...
          <button type="button" class="dp-home-vacation-button${vacationOpen ? ' active' : ''}" data-home-action="vacation">🌴 Urlaubswunsch</button>
      ...
      <section id="dpDriverVacationHost"${vacationOpen ? '' : ' hidden'}>
        <div class="dp-driver-vacation-loading">Urlaubswünsche werden geladen …</div>
      </section>`;
  }
  ...
  function notifyVacationOpen() {
    const host = document.getElementById('dpDriverVacationHost');
    if (!host || host.hidden) return;
    window.dispatchEvent(new CustomEvent('dienstpilot:open-vacation-request', { detail: { profile: profileName(), hostId: host.id } }));
  }

  function bind(panel) {
    panel.querySelectorAll('[data-date]').forEach((button) => {
      button.addEventListener('click', () => { selectedDate = button.dataset.date || selectedDate; render(); });
    });
    ...
    panel.querySelectorAll('[data-home-action]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        if (action === 'today') { weekOffset = 0; selectedDate = localIso(); render(); }
        else if (action === 'tomorrow') { weekOffset = 0; selectedDate = addDays(localIso(), 1); render(); }
        else if (action === 'next') { const next = nextDutyDate(localIso()); if (next) selectedDate = next; render(); }
        else if (action === 'vacation') {
          vacationOpen = !vacationOpen;
          render();
          if (vacationOpen) setTimeout(() => document.getElementById('dpDriverVacationHost')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
        } else if (action === 'print') document.getElementById('printDutyPlan')?.click();
      });
    });
  }
  ...
  function render() {
    ...
    bind(panel);
    requestRemotePlan();
    if (vacationOpen) queueMicrotask(notifyVacationOpen);
    return true;
  }
