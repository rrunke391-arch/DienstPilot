(() => {
  ...
  const TIMES = {
    '3001': ['05:03', '12:12'], '3003': ['05:47', '14:10'], '3005': ['05:51', '15:49'],
    '3006': ['06:00', '16:20'], '3007': ['06:03', '14:19'], '3009': ['06:04', '16:25'],
    '3011': ['06:23', '17:00'], '3012': ['06:31', '16:50'], '3013': ['06:35', '17:05'],
    '3014': ['06:35', '15:39'], '3016': ['06:43', '18:06'], '3019': ['06:49', '17:28'],
    '3022': ['12:03', '19:21'], '3023': ['12:03', '20:21'], '3024': ['12:20', '21:05'],
    '3025': ['13:10', '21:50'], '3095': ['20:20', '04:05']
  };

  const FRIDAY_TIMES = {
    '3005': ['05:51', '15:49'], '3006': ['06:00', '14:21'], '3007': ['06:03', '14:19'],
    '3009': ['06:04', '15:30'], '3011': ['06:23', '14:34'], '3019': ['06:49', '15:50']
  };
  ...
  ready(() => {
    ...
    document.addEventListener('change', (event) => {
      if (event.target && event.target.id === 'kollegeSelect') {
        event.preventDefault();
        loadSelectedKollege();
      }
    }, true);
  ...
  function installExtraStyles() {
    ...
    style.textContent = `
      .kollegen-panel { display:inline-flex; align-items:center; gap:8px; padding:4px; border-radius:14px; }
      .kollegen-panel span { font-weight:800; font-size:14px; }
      .kollegen-panel select { min-width:165px; padding:10px 12px; border-radius:14px; border:1px solid #cbd5e1; background:#fff; font-weight:800; color:#020617; }
      .jahresurlaub-dienstplan-badge { display:inline-flex; align-items:center; gap:4px; margin-left:6px; white-space:nowrap; }
      .day-group.jahresurlaub-day > summary { box-shadow: inset 4px 0 0 #16a34a; }
    `;
    ...
    for (const [start, end, names] of PLAN_ROWS) {
      ...
      while (d <= last) {
        ...
        for (let col = 0; col < names.length; col++) {
          ...
          const number = COLUMN_PATTERNS[col] && COLUMN_PATTERNS[col][weekday];
  ...
  function makeDuty(profile, date, number, index) {
    ...
    return {
      id: `${profile}-${String(index).padStart(3, '0')}-${date}-${number}`,
      date,
      number,
      start: time ? time[0] : '',
      end: time ? time[1] : '',
  ...
  function profileToPlanName(profile) {
    const map = {
      yasar: 'yasar', bumhoffer: 'bumhoffer', entrup: 'entrup', schweppe: 'schweppe', janzen: 'janzen',
      alomar: 'alomar', 'al-sayek': 'alsayek', szczepanik: 'szczepanik', seidensticker: 'seidensticker',
      kocdemir: 'kocdemir', wuellner: 'wullner', wittwer: 'wittwer', gerding: 'gerding', runke: 'runke',
      lommel: 'lhommel', malko: 'malko', murad: 'murad', kurta: 'kurta', wiemann: 'wiemann'
    };
  ...
  function getExistingCustomCatalog() {
    try {
      const state = JSON.parse(localStorage.getItem('lenkRuhezeitenRunke20260413') || '{}');
  ...
  function activeProfileFromLocal() {
    try {
      const state = JSON.parse(localStorage.getItem('lenkRuhezeitenRunke20260413') || '{}');
      return state.appSettings && state.appSettings.activeProfile;
    } catch {
      return null;
    }
  }
  ...
  function updateKollegenTitelOnly() {
    const active = activeProfileFromLocal();
    if (active) setKollegenTitel(active);
  }
  ...
  function refreshJahresurlaub() {
    document.querySelectorAll('#openUrlaubswunsch, #openJahresurlaub').forEach(btn => {
